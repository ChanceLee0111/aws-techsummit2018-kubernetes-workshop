...
Create admin user in kube-system namespace
    kubectl create serviceaccount admin -n kube-system (serviceaccount "admin" created)
    kubectl create clusterrolebinding cluster-admin:kube-system:admin --clusterrole=cluster-admin --serviceaccount=kube-system:admin (clusterrolebinding.rbac.authorization.k8s.io "cluster-admin:kube-system:admin" created)

Copy admin-token from Secret and paste at dashboard token
